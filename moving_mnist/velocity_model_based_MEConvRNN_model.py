...
class MEConvRNNCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size=3, bias=True):
        ...
        # Vanilla recurrence: one conv producing the single candidate state,
        # not four gates. No forget-gate bias to initialise either -- there is
        # no gate to bias, the state is fully overwritten every step.
        self.conv = nn.Conv2d(
    ...
    def forward(self, x, h, u):
        ...
        h = torch.tanh(self.conv(torch.cat([x_exp, h], dim=1)))

        return h.view(B, K, Ch, H, W)

    def init_hidden(self, B, K, H, W, device, dtype):
        return torch.zeros(B, K, self.hidden_dim, H, W, device=device, dtype=dtype)


class Seq2SeqMEConvRNN(nn.Module):
    """
    Encoder-decoder video predictor with K independently tracked velocity slots.

    Vanilla-RNN twin of Seq2SeqMEConvLSTM: same slot/velocity machinery, same
    encoder/decoder protocols, but the recurrent cell carries only h (no cell
    state, no gates), so exactly one tensor per slot is warped each step.
    ...
    def __init__(self,
        ...
        self.cell = MEConvRNNCell(input_channels, hidden_channels,
                                  kernel_size, bias)
    ...
    def forward(self,
        ...
        h = self.cell.init_hidden(B, K, H, W, input_seq.device, input_seq.dtype)
        ...
        for t in range(T_in):
            ...
            if t == 0:
                # h=0, warp(0,v)=0 for any v. h_1 = tanh(U★X_0).
